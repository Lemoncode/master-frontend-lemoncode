...
// En caso de no especificar todas las propiedades lanzará un error de
// compilación
const pos1: Geoposition = { // [ts] Type '{ latitude: number; }' is not assignable to type 'Geoposition'.
  latitude: 3.3112,
};
...
// Vemos que no tenemos problemas a la hora de usarlo con un objeto de
// tipo User
printUserName(julia);

// Pero tampoco hay problemas a la hora de utilizarlo con un objeto de
// tipo Dog
printUserName(laika);

// Esto es, por lo que decimos, User y Dog comparten la misma estructura,
// si intentamos usarlo con otra interfaz que no comparta sus mismos 
// atributos dará un error de tipos
interface Cat {
  name: string;
}

const garfield: Cat = { name: "Garfield" };
printUserName(garfield); // Garfield no tiene "id: number" por lo que no compatible con el tipo que necesita
...
printUserName(bob); // bob al tener "id: number" y "name: string" es compatible, pese a tener más propiedades de las requeridas
